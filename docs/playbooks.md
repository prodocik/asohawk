...
**When:** a decision turns on how many people actually search a term, and the popularity you have is still the free estimate. This uses the Apify account connected to the current ASOHawk workspace, not an external Apify task.
...
3. Relay the explicit hand-off — an agent cannot start the run. The agent spending cap is $0: `snapshot_asa_popularity` without `estimate_only` prices the set, then refuses with `ASOHawk → Keywords → Run ASA snapshot` and confirms it uses the active Apify connection in this workspace. A human starts it there. ASOHawk rejects a selection above $5 and Apify enforces that hard $5 maximum across every actor call in the run. Runs are also throttled to three an hour per workspace; the worker re-checks that each keyword is still tracked immediately before it spends, so a keyword archived in the meantime makes the run cheaper, never dearer.
